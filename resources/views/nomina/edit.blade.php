            ...
            <div class="container mx-auto py-6 bg-white bg-opacity-80 p-6 rounded-lg shadow-lg">
                <form action="{{ route('nomina.update', $nomina->id) }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <select name="empleado_id" id="empleado_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                            @foreach($empleados as $empleado)
                                <option value="{{ $empleado->id }}" {{ $nomina->empleado_id == $empleado->id ? 'selected' : '' }}>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="salario" id="salario" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" value="{{ old('salario', $nomina->salario) }}" required>
                        @error('salario')
                            <span class="text-red-500 text-sm">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="date" name="fecha_pago" id="fecha_pago" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" value="{{ old('fecha_pago', $nomina->fecha_pago->format('Y-m-d')) }}" required>
                        @error('fecha_pago')
                            <span class="text-red-500 text-sm">{{ $message }}</span>
                        @enderror
                    </div>

                    <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded-md hover:bg-blue-600 transition duration-200">Actualizar Nómina</button>
