            ...
            <div class="container mx-auto py-6 bg-white bg-opacity-80 p-6 rounded-lg shadow-lg">
                ...
                <form action="{{ route('permisos.update', $permiso) }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <select name="empleado_id" id="empleado_id" class="block w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring focus:ring-blue-300" required>
                            @foreach($empleados as $empleado)
                                <option value="{{ $empleado->id }}" {{ $empleado->id == $permiso->empleado_id ? 'selected' : '' }}>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="tipo" id="tipo" value="{{ $permiso->tipo }}" class="block w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring focus:ring-blue-300" required>
                    </div>

                    <div class="mb-4">
                        <label for="fecha_inicio" class="block text-gray-700 font-medium mb-2">Fecha de Inicio</label>
                        <input type="date" name="fecha_inicio" id="fecha_inicio" value="{{ $permiso->fecha_inicio }}" class="block w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring focus:ring-blue-300" required>
                    </div>

                    <div class="mb-4">
                        <label for="fecha_fin" class="block text-gray-700 font-medium mb-2">Fecha de Fin</label>
                        <input type="date" name="fecha_fin" id="fecha_fin" value="{{ $permiso->fecha_fin }}" class="block w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring focus:ring-blue-300" required>
                    </div>

                    <button type="submit" class="w-full bg-blue-500 text-white py-2 rounded-md hover:bg-blue-600 transition duration-200">Actualizar</button>
