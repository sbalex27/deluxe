            ...
            <div class="container mx-auto py-6 bg-white bg-opacity-80 p-6 rounded-lg shadow-lg">
                <form action="{{ route('prestamos.store') }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number" name="monto" id="monto" class="block w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring focus:ring-blue-300" required step="0.01">
                    </div>

                    <!-- Fecha de Otorgamiento -->
                    <div class="mb-4">
                        <label for="fecha_otorgamiento" class="block text-gray-700 font-medium mb-2">Fecha de Otorgamiento</label>
                        <input type="date" name="fecha_otorgamiento" id="fecha_otorgamiento" class="block w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring focus:ring-blue-300" required>
                    </div>

                    <!-- Botones -->
                    <button type="submit" class="bg-yellow-400 text-black px-2 py-1 rounded-md hover:bg-blue-500 transition">Guardar</button>
                    <a href="{{ route('prestamos.index') }}" class="bg-yellow-400 text-black px-2 py-1 rounded-md hover:bg-blue-500 transition">Cancelar</a>
                </form>
            </div>
        </div>
    </div>
